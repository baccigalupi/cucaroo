...
class StepCollection {
  constructor(compiled, world, parentScenario) {
    this.compiled = compiled;
    this.world    = world;
    this.status   = world.status;
    this.log      = world.logger;
    this.definitions    = world.stepDefinitionCollection;
  ...
  marry() {
    return this.compiled.map((compiledStep) => {
      ...
      return new StepCollection.Step(compiledStep, matchingDefinitions, this);
    });
  }
  ...
  updateScenarioStatus() {
    if (!this.isHalted()) {
      this.parentScenario.error = true;
    }
  }
}
