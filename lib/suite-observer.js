...
class SuiteObserver {
  ...
  substepNotRun(item, err) {
    ...
    if (this.currentStep !== this.subParent) {
      this.currentStep.emit('not-run', this.currentStep, err, true);
    }
  ...
  substepPending(item, err) {
    if (this.currentStep !== this.subParent) {
      this.currentStep.emit('substep-not-run', this.currentStep, err);
    }
    this.printStep(item, 'pending', true);
    this.subParent = this.currentStep;
  }

  substepFail(item, err) {
    if (this.currentStep !== this.subParent) {
      this.currentStep.emit('fail', this.currentStep);
    }
    this.currentStep.emit('fail', item, err);
    //this.stepFail(item, err, true);
    this.subParent = this.currentStep;
  }
